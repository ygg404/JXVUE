<template>
    <v-container fluid>
        ...
        <v-card>
            <v-card-title class="headline">质量检查</v-card-title>
            <v-card-title>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            editQuality:function(item){
                this.$router.push({name:'editquality',query:{'p_no':item.projectNo,'p_group':item.groupId}});
            },
            getTypeFromApi(){
                return new Promise((resolve,reject) =>{
                    axios({
